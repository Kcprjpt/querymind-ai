...
const STYLE = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pulse { width: 6px; height: 6px; border-radius: 50%; background: var(--green); animation: blink 1.5s infinite; display:inline-block; }
  ...
  .btn-primary { background: linear-gradient(135deg, var(--accent), var(--accent2)); color: white;
    border: none; padding: 14px 28px; border-radius: 10px; font-size: 15px; font-weight: 600;
    cursor: pointer; transition: all 0.2s; box-shadow: 0 0 24px var(--glow); font-family: 'DM Sans', sans-serif; }
  .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 0 40px var(--glow); }
  .btn-outline { background: transparent; color: var(--text); border: 1px solid var(--border);
    padding: 14px 28px; border-radius: 10px; font-size: 15px; cursor: pointer; transition: all 0.2s; font-family: 'DM Sans', sans-serif; }
  .btn-outline:hover { border-color: var(--accent); color: var(--accent); }
  .stats { display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; padding: 0 0 60px; }
  .stat-card { background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 20px 32px; text-align: center; }
  ...
  .section-title { font-family: 'Syne', sans-serif; font-size: 32px; font-weight: 800; text-align: center; margin-bottom: 8px; }
  ...
  .feat-card { background: var(--card); border: 1px solid var(--border); border-radius: 16px;
    padding: 28px; transition: all 0.3s; }
  .feat-card:hover { border-color: var(--accent); transform: translateY(-4px); box-shadow: 0 8px 32px rgba(59,130,246,0.15); }
  ...
  .price-card { background: var(--card); border: 1px solid var(--border); border-radius: 20px;
    padding: 32px 28px; width: 280px; position: relative; transition: all 0.3s; }
  ...
  .price-plan { font-size: 12px; color: var(--muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 12px; }
  ...
  .price-features li { font-size: 13px; padding: 7px 0; border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: 8px; }
  .price-features li:last-child { border-bottom: none; }
  .check { color: var(--green); }
  .btn-plan { width: 100%; padding: 12px; border-radius: 10px; font-weight: 600; font-size: 14px;
  ...
  .sidebar { background: var(--card); border-right: 1px solid var(--border); padding: 24px 0; display: flex; flex-direction: column; }
  .sidebar-section { padding: 0 16px; margin-bottom: 24px; }
  .sidebar-label { font-size: 10px; text-transform: uppercase; letter-spacing: 1.5px; color: var(--muted); margin-bottom: 8px; padding: 0 8px; }
  .sidebar-item { display: flex; align-items: center; gap: 10px; padding: 10px 8px; border-radius: 8px;
    cursor: pointer; font-size: 13px; color: var(--muted); transition: all 0.2s; margin-bottom: 2px; }
  ...
  .chat-messages { height: 380px; overflow-y: auto; display: flex; flex-direction: column; gap: 16px; margin-bottom: 16px; padding: 4px; }
  .chat-messages::-webkit-scrollbar { width: 4px; }
  .chat-messages::-webkit-scrollbar-thumb { background: var(--border); }
  .msg { display: flex; gap: 12px; align-items: flex-start; }
  .msg.user { flex-direction: row-reverse; }
  .msg-avatar { width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center;
  ...
  .msg-bubble pre { background: rgba(0,0,0,0.4); border-radius: 8px; padding: 12px; overflow-x: auto;
    font-size: 12px; margin: 10px 0; white-space: pre-wrap; border: 1px solid var(--border); }
  ...
  .chat-input { flex: 1; background: var(--card); border: 1px solid var(--border); border-radius: 10px;
    padding: 12px 16px; color: var(--text); font-family: 'DM Sans', sans-serif; font-size: 14px; outline: none; resize: none; }
  .chat-input:focus { border-color: var(--accent); }
  .chat-input::placeholder { color: var(--muted); }
  .chat-send { background: linear-gradient(135deg, var(--accent), var(--accent2)); color: white;
    border: none; padding: 12px 20px; border-radius: 10px; cursor: pointer; font-size: 18px; transition: all 0.2s; }
  ...
  .qp { background: rgba(59,130,246,0.08); border: 1px solid rgba(59,130,246,0.2); border-radius: 20px;
    padding: 6px 14px; font-size: 12px; cursor: pointer; color: var(--accent2); transition: all 0.2s; font-family: 'DM Sans',sans-serif; }
  ...
  .sql-textarea { width: 100%; min-height: 120px; background: rgba(0,0,0,0.4); border: 1px solid var(--border);
    border-radius: 8px; padding: 12px; color: var(--text); font-family: 'Courier New', monospace; font-size: 13px; resize: vertical; outline: none; }
  .sql-textarea:focus { border-color: var(--accent); }
  .sql-output { background: rgba(0,0,0,0.5); border: 1px solid var(--border); border-radius: 8px; padding: 12px;
    min-height: 200px; font-family: 'Courier New', monospace; font-size: 12px; color: var(--accent2); white-space: pre-wrap; overflow-x: auto; line-height: 1.6; }
  .generate-btn { width: 100%; margin-top: 12px; padding: 11px; background: linear-gradient(135deg, var(--accent), var(--accent2));
    color: white; border: none; border-radius: 8px; font-weight: 600; font-size: 14px; cursor: pointer; font-family: 'DM Sans',sans-serif; }
  ...
  .calc-input { background: rgba(0,0,0,0.3); border: 1px solid var(--border); border-radius: 8px; padding: 8px 12px;
    color: var(--text); width: 110px; text-align: right; font-family: 'DM Sans',sans-serif; font-size: 14px; outline: none; }
  .calc-input:focus { border-color: var(--accent); }
  .total-box { background: linear-gradient(135deg, rgba(59,130,246,0.08), rgba(6,182,212,0.08));
    border: 1px solid rgba(59,130,246,0.3); border-radius: 12px; padding: 24px; text-align: center; margin-top: 20px; }
  ...
  .tag { display: inline-flex; align-items: center; background: rgba(16,185,129,0.1);
    border: 1px solid rgba(16,185,129,0.25); color: var(--green); border-radius: 6px; padding: 2px 8px; font-size: 11px; font-weight: 600; }
...
const FEATURES = [
  { icon: "🧠", bg: "rgba(59,130,246,0.15)", title: "Natural Language to SQL", desc: "Type your question in plain English — QueryMind instantly converts it to a production-ready SQL query. No coding required." },
  { icon: "📊", bg: "rgba(16,185,129,0.15)", title: "Real-time Business Insights", desc: "Sales trends, revenue patterns, customer behavior — AI analyzes your data and delivers clear, actionable insights on demand." },
  { icon: "🔮", bg: "rgba(139,92,246,0.15)", title: "Predictive Analytics", desc: "Revenue forecasting, churn prediction, inventory planning — know what's coming before it happens with AI-powered predictions." },
  { icon: "📈", bg: "rgba(245,158,11,0.15)", title: "Automated Reports", desc: "Schedule daily or weekly PDF reports. They auto-generate and land in your inbox every morning — zero manual effort." },
  { icon: "🔗", bg: "rgba(239,68,68,0.15)", title: "100+ Integrations", desc: "Connect MySQL, PostgreSQL, Snowflake, Shopify, Salesforce, and more — all unified in a single intelligence layer." },
  { icon: "🛡️", bg: "rgba(6,182,212,0.15)", title: "Enterprise Security", desc: "Your data stays in your region. SOC 2 certified, GDPR compliant, and end-to-end encrypted at rest and in transit." },
];
...
export default function QueryMindAI() {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `You are QueryMind AI — an intelligent Business Intelligence assistant for SMBs and enterprises.
You help users analyze business data, generate optimized SQL queries, and surface actionable insights.
Respond professionally and concisely. When generating SQL, always use a code block with proper formatting.
Assume a standard e-commerce database: orders, products, customers, transactions tables.
Always end with one relevant follow-up suggestion.`,
      ...
      setMessages(prev => [...prev, { role: "ai", text: "Network error. Please check your connection and retry." }]);
    }
    setLoading(false);
  };
  ...
  const generateSQL = async () => {
    ...
    setSqlLoading(true); setSqlOutput("");
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514",
          max_tokens: 1000,
          system: `You are a senior SQL expert. Convert natural language to optimized, production-ready SQL.
Return ONLY the SQL query with inline comments explaining each section. Use PostgreSQL syntax.
...
transactions(id, order_id, payment_method, amount, created_at).`,
          messages: [{ role: "user", content: `Convert to SQL: ${sqlInput}` }]
        })
      });
      const data = await res.json();
      setSqlOutput(data.content?.[0]?.text || "Error generating SQL.");
    } catch { setSqlOutput("Error: Network issue. Please retry."); }
    setSqlLoading(false);
  };
        ...
        {tab==="app" && (
          <div className="app-container">
            <div className="sidebar">
              ...
              <div className="sidebar-section" style={{marginTop:"auto"}}>
                <div style={{padding:"12px",background:"rgba(59,130,246,0.08)",borderRadius:"10px",border:"1px solid rgba(59,130,246,0.2)"}}>
                  ...
                  <div style={{fontSize:"11px",color:"var(--muted)",marginTop:"2px"}}>$99/month</div>
                </div>
              </div>
            </div>
            <div className="main-content">
              {subTab==="chat" && (
                <div>
                  ...
                  <div className="quick-prompts">
                    {QUICK_PROMPTS.map(p=><button key={p} className="qp" onClick={()=>sendMessage(p)}>{p}</button>)}
                  </div>
                  <div style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:"16px",padding:"20px"}}>
                    <div className="chat-messages">
                      {messages.map((m,i)=>(
                        <div key={i} className={`msg ${m.role==="user"?"user":""}`}>
                          <div className={`msg-avatar ${m.role==="user"?"user-av":"ai"}`}>{m.role==="user"?"👤":"🤖"}</div>
              ...
              {subTab==="dashboard" && (
                <div>
                  <div className="chat-header"><h2>Business Dashboard 📊</h2><p>Real-time metrics — your entire business at a glance</p></div>
                  <div className="metrics-grid">
                    {[
                      {label:"Revenue (MTD)",value:"$84,230",change:"+18.4% vs last month",up:true},
                      {label:"Active Customers",value:"1,247",change:"+34 this week",up:true},
                      {label:"Avg Order Value",value:"$284",change:"-2.1% vs last week",up:false},
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px"}}>
                    {[
                      {title:"Top Products 🔥",cols:["Product","Revenue","Units"],rows:[["Premium Widget","$12,450","45"],["Basic Kit","$8,920","102"],["Pro Bundle","$7,680","28"]]},
                      {title:"Recent Orders 📦",cols:["Order ID","Customer","Amount"],rows:[["#ORD-8821","Alex Johnson","$420"],["#ORD-8820","Sarah Chen","$890"],["#ORD-8819","Mike Patel","$210"]]},
                    ].map(panel=>(
                      <div key={panel.title} style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:"12px",padding:"20px"}}>
                        ...
                        {panel.rows.map(row=>(
                          <div key={row[0]} style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",padding:"9px 0",borderTop:"1px solid var(--border)",fontSize:"13px"}}>
                            <span>{row[0]}</span><span style={{color:"var(--muted)"}}>{row[1]}</span>
              ...
              {subTab==="sql" && (
                <div>
                  <div className="chat-header"><h2>SQL Generator 🔧</h2><p>Describe what you want in plain English — get production-ready SQL instantly</p></div>
                  <div className="sql-container">
                    <div className="sql-panel">
                      <h3>📝 Your Question</h3>
                      <textarea className="sql-textarea" value={sqlInput} onChange={e=>setSqlInput(e.target.value)}
                        placeholder="e.g. Find top 10 customers by total spend this quarter with their order count..."/>
                    ...
                    <div className="sql-panel">
                      ...
                      <div className="sql-output">{sqlLoading?"Generating optimized query...":sqlOutput||"← Enter your question and click Generate"}</div>
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="scrollable">
            <div style={{maxWidth:"600px",margin:"0 auto",padding:"40px 20px"}}>
              <div style={{textAlign:"center",marginBottom:"32px"}}>
                ...
                <p style={{color:"var(--muted)",marginTop:"8px"}}>Model your SaaS revenue potential with QueryMind</p>
              </div>
              <div className="calc-box">
                <h3>📊 Estimate Your Monthly Revenue</h3>
                <div className="calc-row">
                  <span className="calc-label">Number of Paying Customers</span>
                  <input type="number" className="calc-input" value={customers} min={1} onChange={e=>setCustomers(Number(e.target.value))}/>
                </div>
                <div className="calc-row">
                  <span className="calc-label">Average Plan Price ($/mo)</span>
                  <input type="number" className="calc-input" value={planPrice} min={1} onChange={e=>setPlanPrice(Number(e.target.value))}/>
                </div>
                <div className="calc-row">
                ...
                <div className="total-box">
                  ...
                  <div className="total-amount">${mrr.toLocaleString()}</div>
                  <div style={{fontSize:"14px",color:"var(--muted)",marginTop:"8px"}}>
                    Annual Recurring Revenue: <strong style={{color:"var(--green)"}}>${arr.toLocaleString()}</strong>
                  </div>
                </div>
                <div style={{marginTop:"20px",padding:"16px",background:"rgba(16,185,129,0.06)",borderRadius:"12px",border:"1px solid rgba(16,185,129,0.2)"}}>
                  <div style={{fontSize:"13px",fontWeight:"600",color:"var(--green)",marginBottom:"12px"}}>📈 Growth Roadmap to $25K+/month:</div>
                  {[
                    ["Month 1–3","10–20 customers · $1K–2K/mo","Beta launch, referrals, LinkedIn outreach"],
                    ["Month 4–6","50–100 customers · $5K–10K/mo","Content marketing + cold email campaigns"],
                    ["Month 7–12","200–400 customers · $20K–40K/mo","Paid ads + affiliate partnerships"],
                    ["Year 2+","1000+ customers · $100K+/mo","Enterprise deals + white-label resellers"],
                  ].map(([phase,rev,how])=>(
                    <div key={phase} style={{padding:"10px 0",borderBottom:"1px solid rgba(16,185,129,0.12)",fontSize:"12px"}}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:"3px"}}>
                        ...
                        <span style={{color:"var(--green)",fontWeight:"700"}}>{rev.split("·")[1]}</span>
                      </div>
                      <div style={{color:"var(--muted)"}}>{rev.split("·")[0]} — {how}</div>
                    </div>
                  ))}
                </div>
